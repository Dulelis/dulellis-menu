            ...
            <div className="grid gap-4 lg:grid-cols-2">{(data?.produtos || []).map((product) => {
              ...
              return (
                <article key={product.id} className="rounded-3xl bg-white p-5 shadow-sm">
                  <div className="flex items-start justify-between gap-3"><div className="min-w-0 flex-1"><input value={product.nome} onChange={(event) => updateProduct(product.id, { nome: event.target.value })} className="w-full rounded-lg border border-transparent text-lg font-black outline-none focus:border-pink-200" /><div className="mt-2 grid grid-cols-2 gap-2"><input value={product.categoria || ""} onChange={(event) => updateProduct(product.id, { categoria: event.target.value })} placeholder="Categoria" className="rounded-xl border p-2 text-xs font-bold" /><input type="number" min="0" step="0.01" value={Number(product.preco || 0)} onChange={(event) => updateProduct(product.id, { preco: Number(event.target.value) })} className="rounded-xl border p-2 text-xs font-bold" /></div></div><label className="flex shrink-0 items-center gap-2 text-xs font-black uppercase"><input type="checkbox" checked={product.disponivel_encomenda === true} onChange={(event) => updateProduct(product.id, { disponivel_encomenda: event.target.checked })} />Encomenda</label></div>
